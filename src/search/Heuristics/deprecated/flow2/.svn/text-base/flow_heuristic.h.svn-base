#ifndef FLOW2_HEURISTIC_H
#define FLOW2_HEURISTIC_H
...
class Heuristic : public ::Heuristic {
    ...
    mutable std::vector<bool> checked_operators_; // primitive operators that had been processed and can be obviated
    ...
    std::list<std::vector<int> >* find_flaws_in_current_solution(const State &state) const;
...
std::ostream& operator<<(std::ostream &os, const Flow2::Variable &var) {
    var.dump(os);
    return os;
}

std::ostream& operator<<(std::ostream &os, const Flow2::Proposition &prop) {
    prop.dump(os);
    return os;
}

std::ostream& operator<<(std::ostream &os, const Flow2::Operator &op) {
    op.dump(os);
    return os;
}
