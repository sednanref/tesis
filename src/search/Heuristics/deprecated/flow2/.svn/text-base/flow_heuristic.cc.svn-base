...
list<vector<int> > *Heuristic::find_flaws_in_current_solution(const State &state) const {
    list<vector<int> > *flaws = new list<vector<int> >;

    // every prevail condition is a flaw
    for( int i = 0; i < nprimitive_operators_; ++i ) {
        const Operator &op = *operators_[i];
        ...
        if( !checked_operators_[i] && must_be_accounted_for ) {
            flaws->push_back(vector<int>(1, i));
            checked_operators_[i] = true;
            //cout << "Operator " << op << " scheduled for repair due to prevail" << endl;
        }
    }

    // find flaws due to cyclic dependencies in current solution. For this, first need
    // to construct dependency graph among (primitive) actions in current solution and
    // then find cycles in the graph. Edges of the graph are between producer/consumer
    // of fixed fluent. The edges are labeled with the fluent. To account for given
    // state, one edge for a consumed fluent that is true in given state is not added.
    vector<int> budget(nprimitive_propositions_, 0);
    for( int i = 0; i < nprimitive_variables_; ++i ) {
        budget[primitive_propositions_[i][state[i]]->id_] = 1;
    }

    Digraph::Directed dg(nprimitive_operators_, 0, 0);
    for( int i = 0; i < nprimitive_operators_; ++i ) {
        if( operators_active_in_solution_[i] ) {
            const Operator &producer = *operators_[i];
            for( int j = 0; j < producer.produces_.size(); ++j ) {
                const Proposition &prop = *producer.produces_[j];
                for( int k = 0; k < prop.consumed_by_.size(); ++k ) {
                    int pid = prop.consumed_by_[k]->id_;
                    if( (pid < nprimitive_operators_) && operators_active_in_solution_[pid] ) {
                        // add edge between producer and consumer labeled w/ proposition id
                        if( budget[prop.id_] == 0 ) {
                            //cout << "add edge: " << producer.id_ << " --> " << pid << " by " << prop << endl;
                            dg.add_edge(producer.id_, pid, prop.id_);
                        } else {
                            //--budget[prop.id_];
                        }
                    }
    ...
    vector<int> parent, forward, backward;
    dg.dfs(parent, forward, backward);
    //dg.dump(cout);
    cout << "Cycles, #=" << backward.size() << endl;
    for( int i = 0; i < backward.size(); ++i ) {
        pair<int, int> edge = dg.get_edge(backward[i]);
        cout << "  cycle:";
        cout << endl;
    }
    return flaws;
...
bool Heuristic::refine_model(const State &state) {
    ...
    while( change ) {
        ...
        // find flaws in current solution and fix them
        list<vector<int> > *flaws = find_flaws_in_current_solution(state);
        if( flaws->empty() ) { delete flaws; break; }
        for( list<vector<int> >::const_iterator it = flaws->begin(); it != flaws->end(); ++it ) {
            const vector<int> &flaw = *it;

            // fix flaws due to prevail conditions
            if( flaw.size() == 1 ) {
                Operator &op = *operators_[flaw[0]];
                //cout << "Fixing " << op << ": #prevails=" << op.prevails_.size() << ", #consumed=" << op.consumes_.size() << ", #produced=" << op.produces_.size() << endl;
                for( int j = 0; j < op.prevails_.size(); ++j ) {
                    Proposition *first = op.prevails_[j];
                    if( first->id_ >= nprimitive_propositions_ ) continue;
                    //cout << "Target proposition: " << *first << endl;
            ...
            // fix flaws for non-trivial cycles; i.e., not due to prevails
...
bool Heuristic::merge_propositions(Proposition *first, Proposition *second) {
    ...
    for( int i = 0; i < first->prevail_of_.size(); ++i ) {
        ...
        if( second->is_prevail_of(op) ) {
            ...
            //np->prevail_of_.push_back(op);
            //op->prevails_.push_back(np);
    ...
    for( int i = 0; i < first->produced_by_.size(); ++i ) {
        ...
        if( first->is_mutex_with_precondition_of(op) ) {
            // First didn't hold before applying action.
            if( second->is_prevail_of(op) || second->is_produced_by(op) ) {
                // Operator produces merge.
                np->produced_by_.push_back(op);
                op->produces_.push_back(np);
            } else if( second->is_consumed_by(op) ) {
                // Merge didn't and won't hold after action, so do nothing
            } else {
                // Second isn't affected by operator. Make a copy that produces
                // merge (if second isn't mutex with postcondition)
                if( !second->is_mutex_with_postcondition_of(op) ) {
                    if( !add_copy_producer(op, np) ) goto fin;
                }
            }
        } else {
            // First isn't mutex with precondition and thus it could hold before
            // applying the action.
            if( second->is_prevail_of(op) ) {
                // Second may hold or not before applying operator. Make a copy
                // that produces it (if second isn't mutex with postcondition)
                if( !add_copy_producer(op, np) ) goto fin;
            } else if( second->is_consumed_by(op) ) {
                // Second is consumed. Make copy that consumes merge.
                if( !add_copy_consumer(op, np) ) goto fin;
            } else if( second->is_produced_by(op) ) {
                if( second->is_mutex_with_precondition_of(op) ) {
                    // Operator produces merge.
                    ...
                    // Second may hold or not before applying operator.
                    // Make a copy that produces it.
                    if( !add_copy_producer(op, np) ) goto fin;
                }
            } else {
                // Second isn't affected. Make a copy that produces merge (if
                // non-mutex with postcondition).
                if( !second->is_mutex_with_postcondition_of(op) ) {
                    if( !add_copy_producer(op, np) ) goto fin;
                }
            }
    ...
    np->produced_by_.reserve(np->produced_by_.size() + second->consumed_by_.size());
    ...
    np->consumed_by_.reserve(np->consumed_by_.size() + second->produced_by_.size());
    ...
    for( int i = 0; i < second->produced_by_.size(); ++i ) {
        ...
        if( !processed[op->id_] ) {
            if( !first->is_mutex_with_postcondition_of(op) ) {
                if( !add_copy_producer(op, np) ) goto fin;
            }
        }
    }
...
Plugin<ScalarEvaluator> _plugin("flow2", _parse);
