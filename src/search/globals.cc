...
void read_axioms(istream &in, bool evaluate_at_init = true) {
    int count;
    in >> count;
    for (int i = 0; i < count; i++)
    ...
    if( evaluate_at_init ) g_axiom_evaluator->evaluate(*g_initial_state);
}
...
void read_everything_except_initial_state(istream &in) {
    read_and_verify_version(in);
    read_metric(in);
    read_variables(in);
    read_mutexes(in);
...
void read_initial_state(istream &in) {
    g_initial_state = new State(in);
