...
void LazySearch::finalize() {
    heuristics.clear();
    estimate_heuristics.clear();
...
void LazySearch::initialize(const State &state) {
    initial_state = &state;
    current_state = new State(*initial_state);
...
int LazySearch::step() {
    ...
    if (node.is_new() || reopen) {
        ...
        if (dummy_address == NULL) {
            dummy_address = const_cast<state_var_t *>(initial_state->get_buffer());
        }
